...
class PipelineSession:
    ...
    async def start(self):
        ...
        async with self._lock:
            if self._generator is None:
                # Just store the coroutine directly, don't await it
                self._generator = self.pipeline.run()  
    ...
    async def __anext__(self):
        ...
        try:
            # First get the actual generator if we're still holding the coroutine
            if asyncio.iscoroutine(self._generator):
                self._generator = await self._generator
