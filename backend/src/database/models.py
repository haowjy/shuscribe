...
class Document(Base):
    ...
    file_tree_id: Mapped[Optional[str]] = mapped_column(String(36), ForeignKey("file_tree_items.id"), nullable=True)
    ...
    file_tree_item: Mapped[Optional["FileTreeItem"]] = relationship("FileTreeItem", back_populates="document", foreign_keys=[file_tree_id])


class FileTreeItem(Base):
    """File tree item model matching frontend FileTreeItem interface"""
    __tablename__ = "file_tree_items"
    ...
    path: Mapped[str] = mapped_column(String(500), nullable=False)
    parent_id: Mapped[Optional[str]] = mapped_column(String(36), ForeignKey("file_tree_items.id"), nullable=True)
    ...
    document: Mapped[Optional["Document"]] = relationship("Document", back_populates="file_tree_item", foreign_keys=[Document.file_tree_id])