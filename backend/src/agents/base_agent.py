...
class BaseAgent(ABC):
    ...
    def _validate_default_model(self) -> None:
        ...
        """
        model_instance = self.llm_service.get_hosted_model_details(
            self.default_provider, 
            self.default_model
        )
        
        if not model_instance:
            available_models = self.llm_service.get_hosted_models_for_provider(self.default_provider)
    ...
    async def _make_llm_call(
        ...
        **llm_kwargs
        ...
        """
        Make an LLM call with consistent error handling.
